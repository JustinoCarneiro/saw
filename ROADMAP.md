...
| 20 | E14/E17 · Financeiro & Painel Consolidado — itens do "Sequenciamento sugerido" da reunião 17/07/2026 (change request pós-MVP) | Médio (parcial) | — | ✅ **Concluído — E14 7/7 itens, E17 4/4 itens** (todos os itens listados nesta linha estão fechados; ver linhas 21 e 22 abaixo pro detalhe técnico completo de cada um). E14: filtro mensal em contas a pagar/receber, rename da aba "Faturamento"→"Dashboard", conciliação contrato×recebido, alimentação automática de contas via venda (já vinha do M25), evento rastreado no financeiro, subcategorias/fixo-variável em `CategoriaFinanceira`, e o merge `ContaPagarReceber`→`LancamentoFinanceiro` (M26, linha 21). E17: campo `Ata.decisoes`, e os 3 itens de maior risco — controle de presença em mentoria coletiva, ranking com as 4 ferramentas nomeadas, dois eixos de acompanhamento — fechados no M27 (linha 22, 19/07/2026), decididos sem resposta do Victor às perguntas enviadas, por isso desenhados 100% aditivos. Achado à parte, também fechado: filtro por mentorado no Painel Consolidado (busca por nome client-side); filtro por área segue não implementado, exige decisão de produto (`área` hoje só existe em `Colaborador`, não em `Mentorado`) |
...
| 23 | Financeiro — fusão de "Lançamentos" e "Contas a pagar/receber" numa tela só (change request pós-MVP, 20/07/2026) | Pequeno | ~1d | ✅ **Concluído (20/07/2026)** — achado do Marcos ao revisar o M26: as 2 abas já liam/escreviam a mesma tabela (`LancamentoFinanceiro`) desde o merge de entidade, mas continuavam 2 telas separadas — redundante. Fundidas em `LancamentosPage.tsx`; `ContasPage.tsx` removido, rota `/admin/financeiro/contas` vira redirect pra `/lancamentos` (link salvo não quebra). Backend **não mudou** — `ContaController`/`GET .../contas` continuam existindo (mesmo `LancamentoService` por baixo), só o frontend parou de os chamar; decisão consciente de não expandir escopo pro backend, já que o pedido era sobre a tela. Listagem usa `GET .../lancamentos` (filtro por `dataCompetencia`, sempre preenchida) como base, não `GET .../contas` (filtro por `dataVencimento` — **achado próprio antes de implementar**: essa query nunca bate `NULL`, um lançamento criado direto como Realizado sem vencimento sumiria da tela se essa fosse a base). Filtro de período virou opcional (checkbox "Filtrar por período", desligado por padrão = mostra tudo) — replica o comportamento que só "Contas" tinha; sem isso, uma parcela futura (ex. vencimento daqui a 2 meses) não apareceria por padrão na janela "mês atual" que só "Lançamentos" tinha. Form único de criação: pergunta "Já foi realizado?" (Sim → pede Data de competência, nasce Realizado, sem vencimento; Não → pede Vencimento, nasce Previsto com `dataCompetencia = dataVencimento`, mesmo comportamento que "Nova conta" já tinha). Liquidar/Parcial usam `.../lancamentos/{id}/liquidar[-parcial]` (idêntico a `.../contas/{id}/...`, mesmo código por baixo). `tsc -b` limpo. E2E: `financeiro.spec.ts`/`financeiro-import-export.spec.ts` atualizados (teste de "Contas a pagar/receber" virou cenário "Previsto" dentro de Lançamentos; export de "Contas" removido do teste por ser redundante com o de Lançamentos, mesmo CSV; import de CSV com Previsto+vencimento preservado como cenário próprio); `comercial-venda-m25.spec.ts` (parcelamento) atualizado pra navegar pra "Lançamentos" em vez de "Contas a pagar/receber". 13/13 testes de Financeiro/parcelamento verdes |
| — | **Fase 5 · Homologação** (smoke test via Docker, validação humana E2E, revisão final de segurança, deploy Coolify, **pass transversal de `pgcrypto` nas colunas sensíveis** — ver notas em M04 e M05) | — | 2d | ✅ **Concluída (14/07/2026)**, logo após o MVP inicial e antes de todo o change request M23-M27 — esta linha ficou ⬜ por engano depois disso, nunca foi corrigida (achado, 20/07/2026). Commits: `5665060` (2 bugs de build/boot achados no smoke test — `JSX.Element`→`ReactElement` no React 19, chown do diretório de áudio no Dockerfile), `f6c6d36` (pass transversal de pgcrypto — `lead.*`, `mentorado.telefone/bio`, `mentorado.crescimento_faturamento_pct`, `colaborador.nome`), `246cb32` (pipeline de CI real: `backend-test`/`frontend-build`/`docker-smoke-e2e`), `a17e159` (token de redefinição de senha parava de vazar em log). **Re-verificação em 20/07/2026** (motivada pelo volume de mudança desde então — M23 a M27): pgcrypto continua sem gap (auditado de novo, todo campo sensível novo desde 14/07 seguiu o mesmo critério documentado inline). Achado grave: `docker-smoke-e2e` no CI estava **quebrado desde 18/07/2026** (10 pushes seguidos falhando, nada a ver com o código de cada commit) — `curl -sf` no passo de esperar o backend subir trata `/api/v1/auth/me` (que responde 401 corretamente sem sessão) como falha, o loop nunca saía, sempre estourava os 120s de timeout. Corrigido (`-f` removido, mesmo padrão do `full-up.sh`, que nunca teve esse bug). Achado 2: `docker-compose.full.yml` nunca ganhou os rate limits generosos que `scripts/e2e-up.sh` sempre teve (`APP_RATE_LIMIT_LEAD`/`ATA_AUDIO`/`PASSWORD_RESET`) — a suíte E2E completa soma mais que o limite de produção nesses 3 endpoints públicos; corrigido e reverificado (`comercial.spec.ts` voltou a passar). Achado 3: `docker-compose.full.yml` nunca subiu os stubs de Google OAuth (porta 8092) nem de Whisper/Claude (porta 8091) que `e2e-up.sh` sobe como processo Node à parte — decisão do Marcos: excluir `google-oauth.spec.ts` e o teste de upload de áudio desse job específico via `--grep-invert` (já cobertos de ponta a ponta pelo `e2e-up.sh`), em vez de duplicar infra de stub — `docker-smoke-e2e` existe pra validar a topologia de produção (front+back atrás do Nginx via Coolify), não reintegrações externas. Achado 4, mesmo `--grep-invert`: a área do mentorado/Loja está pausada por feature flag desde 17/07/2026 (`AREA_MENTORADO_PAUSADA`/`LOJA_ADMIN_PAUSADA`) — falha de propósito, excluída também (por describe/arquivo inteiro, não por teste individual — aceito pular ~12 testes que ainda passam, ex. "Admin:"/"isolamento por tenant" via API direta, já cobertos por `e2e-up.sh`). Achado 5: `MAIL_HOST` também nunca foi wireado em `docker-compose.full.yml` — `esqueci-senha.spec.ts` (caminho feliz) sempre falhava com "nenhum e-mail encontrado no Mailpit", não por lentidão, o e-mail nunca era enviado de verdade (`email.host` vazio = fallback sem SMTP real). Corrigido apontando pro serviço `mailpit` do compose (não `localhost`, já que o backend roda em container aqui). **Suíte filtrada (73 testes) 73/73 verde contra stack 100% fresca localmente** (`full-down.sh -v` + `full-up.sh`, mesmo isolamento do CI) antes de cada push, e **CI confirmado verde nos 3 jobs em produção** (`backend-test`/`frontend-build`/`docker-smoke-e2e`, run `822c1ac`, 20/07/2026) — primeira vez em 3 dias. **Revisão final de segurança rodada em seguida** (20/07/2026, `revisor-seguranca` amplo — `SecurityConfig` inteiro, cobertura de rate limit, vazamento de erro, mais as 3 mudanças de infra de hoje): achado 1 (médio, corrigido) — `POST /api/v1/auth/login` era o único dos 4 endpoints públicos sem rate limit (os outros 3 já tinham via `LeadRateLimitFilter`/`AtaAudioRateLimitFilter`/`PasswordResetRateLimitFilter`); `LoginRateLimitFilter` novo, mesmo padrão Redis/janela de 10min, mas conta só tentativa FALHA (não toda requisição como os outros 3 — login legítimo é frequente, contar tudo bloquearia uso normal/E2E) e zera o contador do IP em login bem-sucedido. Verificado manualmente contra backend real (5 falhas → 429, sucesso zera o contador) e suíte E2E completa sem regressão nova. Achado 2 (médio, já conhecido) — `transicao-acessos-para-cliente.md`/`custos-operacionais.md` versionados apesar do CLAUDE.md dizer "arquivo local": removidos do rastreamento (`git rm --cached` + `.gitignore`) a pedido do Marcos — conteúdo permanece em commits antigos já publicados, limpeza de histórico é decisão separada, não feita. Achado ao vivo testando o deploy (baixo, corrigido): os 7 pontos do sistema que respondem JSON via `HttpServletResponse.getWriter()` direto (fora do pipeline Spring MVC normal, que já é UTF-8 por padrão) — `AuthSuccessHandler`/`AuthFailureHandler`/`JsonAuthEntryPoint`/`LeadRateLimitFilter`/`PasswordResetRateLimitFilter`/`AtaAudioRateLimitFilter`/`LoginRateLimitFilter` — declaravam `Content-Type` sem charset; Tomcat cai pro default do Servlet spec (ISO-8859-1), que funciona pra português (Latin-1 cobre a acentuação) mas fica inconsistente com o resto da API. `;charset=UTF-8` explícito nos 7. **Deploy Coolify executado (20/07/2026)**: produção estava 18 commits atrás do master (confirmado `git push` não dispara deploy automático, mesmo achado de sessão anterior) — `saw-hub-backend` e `saw-hub-frontend` deployados via API do Coolify (SSH + token temporário via tinker, procedimento documentado em memória), 22 migrations novas (V23-V44) aplicadas em produção sem erro, verificado via `https://157.173.212.76.sslip.io` (rota real, não a fqdn do Coolify — Traefik continua fora do ar nessa VPS). Backend redeployado uma 2ª vez pro fix de charset. Token revogado ao final. **Achado de config em produção** (baixo impacto real, corrigido): `SEED_DEMO_DATA=true` estava configurado no painel do Coolify pro `saw-hub-backend` — não causou dano porque cada bloco do `DemoDataSeeder` só roda se a tabela correspondente estiver vazia (já havia dado manual mínimo de teste travando todos os blocos), mas era um risco real (truncar qualquer tabela por engano injetaria fixture de demo no banco de produção no próximo boot). Confirmado nesse mesmo raio-x: produção ainda não tem dado real de cliente — só 2 mentorados placeholder ("Teste Demo"/"Demo Apresentacao") e um punhado de registros manuais de teste em outras tabelas (2 leads, 1 mentoria, 15 lançamentos, 4 conteúdos, 4 eventos, 1 produto, 4 avisos, 3 colaboradores). Corrigido pra `false` via API do Coolify + restart (sem rebuild, só a env nova) — verificado dentro do container novo |

| 24 | M28 · Refino da área Mentorados (2 botões de import confusos, seleção de mentorado em lista, página dedicada estilo Notion, reorganização de mentorias, rename "Gestão de Performance", colar transcrição do Meet, cota de eventos grátis) — pedido do Marcos ao revisar o dev (21/07/2026), + 4 pendências do E5/E17/E7 já registradas na reunião de 17/07/2026 | Grande (7 fases) | ~4-5d | ✅ **Concluído (21/07/2026)** — plano completo em `/home/marcos/.claude/plans/magical-floating-kurzweil.md` (fases A-G), implementado sequencialmente fase a fase, todas as 7 fechadas. **Fase A concluída** — import de mentorados unificado num fluxo só: `MentoradoDiretoCsvService` (antes só CREATE) passou a resolver por e-mail e ATUALIZAR quando o mentorado já existe (`MentoradoAdminService.buscarPorEmail`/`existeContaComEmail`/`atualizarDeImportacao` novos), em vez de rejeitar o e-mail como erro; `MentoradoCsvService` perdeu seu `importar()` (bulk-UPDATE estreito de 6 campos, era o segundo botão confuso) e ficou só com `exportar()`. `CsvImportExport` ganhou `importUrl`/`onImportado` opcionais (Mentorados agora só usa o widget pra exportar). Resultado do import ganhou `atualizados` ao lado de `importados`/`criados`. 521/521 testes backend (`MentoradoCsvServiceRepositoryTest` removido — só testava o `importar()` extinto; cobertura de sessão real do Hibernate pro caminho UPDATE migrou pra `MentoradoDiretoCsvServiceRepositoryTest`), `tsc -b` limpo, E2E 20/20 (`mentorados-comercial-import-export.spec.ts` reescrito pro import único, `mentorados-contrato-m23.spec.ts`, `mentorados.spec.ts`). **Fase B concluída** — seleção de mentorado em `NovaMentoriaForm` (Nova Mentoria) trocou de grade CSS (`repeat(auto-fill, minmax(180px,1fr))`, quebra de coluna imprevisível) pra lista vertical (1 por linha); só CSS, verificado visualmente contra o dev real (screenshot). **Fase E concluída** — "Mentorados" renomeado pra "Gestão de Performance" nos 3 lugares visíveis (`Sidebar` nav, `MentoradosShell` Topbar+aba da lista, `TeamPage.MODULO_LABEL`); rotas (`/admin/mentorados/*`) e nomes de classe/arquivo não mudaram, só o label. Colisão de nome com a área RBAC `Colaborador.area=GESTAO_PERFORMANCE` é intencional (confirmado com o Marcos — é o nome que o time já usa). E2E atualizado em 6 arquivos (`login`/`rbac`/`consolidated`/`mentorados`/`mentorados-comercial-import-export`/`mentorados-contrato-m23`), 32/32 verdes.

**Fase C concluída** — `MentoradoDetalhePage.tsx` nova, rota `/admin/mentorados/lista/:id` deliberadamente FORA do bloco `MentoradosShell` em `App.tsx` (sibling route sob `/admin`, dentro de `RequireModulo`) — só a Sidebar do `AdminShell` continua visível, sem as abas Painel Consolidado/Gestão de Performance/Mentorias/Metas/Tarefas por cima, dando o "foco total" pedido ("como se fosse o Notion dele"). Migração 1:1 de `EditarMentoradoForm` e as 4 sub-seções que viviam expandidas inline em `MentoradosListaPage.tsx` (Perfil, Dados de contrato, Diagnóstico Inicial, Ferramentas obrigatórias, Acompanhamento), cada uma agora seu próprio `<Card>` empilhado (mesmo padrão de `AtaDetalhePage.tsx`). "Editar" virou "Ver perfil" (navega em vez de expandir). Novidades: card de métricas no topo (reaproveita `GET /admin/consolidated/mentorados` já existente, filtrado client-side por id — mesmo padrão MVP-scale já usado em `ConsolidatedPage.tsx`, sem endpoint novo) e seção "Mentorias" (histórico individual/grupo do mentorado, "Ver ata" pra realizadas). Backend novo: `GET /admin/mentorados/{id}` (nunca existira — a tela sempre reaproveitava o objeto já carregado pela listagem; `MentoradoAdminService.buscarPorId`) e `GET /admin/mentorias` ganhou `tipo`/`mentoradoId` opcionais (`MentoriaService.listar`, mesma composição em memória de sempre — `mentoradoId` troca a base pra `buscarPorMentorado`, que já existia desde o M08/M12 mas nunca tinha endpoint admin). Achado ao vivo: dev local (`dev-up.sh`) roda `spring-boot:run` sem hot-reload de verdade — precisou restart manual do backend pra pegar os endpoints novos antes da verificação visual (`GET /{id}` batia 405 contra o processo antigo). 529/529 testes backend, `tsc -b` limpo, E2E: spec novo `mentorado-detalhe-m28.spec.ts` (3/3) + `mentorados`/`mentorados-contrato-m23`/`perfil` atualizados pro novo fluxo "Ver perfil" (13+1 verdes, os 5 mentee-facing de `perfil.spec.ts` seguem falhando por `AREA_MENTORADO_PAUSADA`, não-regressão).

**Fase D concluída** — `MentoriasAgendaPage.tsx` ganhou um filtro de tipo (aria-label "Filtro de tipo (agenda)" — deliberadamente sem a palavra "Mentor", achado ao vivo: `getByLabel('Mentor')` do form de criação faz substring match e bateria em "mentor**ia**"), padrão **Grupo**; Individual/Todos os tipos continuam disponíveis pra quem precisar ver junto (não some do sistema, só do padrão). Backend (`tipo`/`mentoradoId` em `GET /admin/mentorias`) já tinha sido feito junto com a Fase C. **Achado ao vivo, corrigido**: condição de corrida real em `MentoriasAgendaPage.tsx` — "Nova mentoria" dispara um POST assíncrono cujo `onCriada` só chama `carregar()` quando o POST termina; se o filtro de tipo mudar ENQUANTO esse POST ainda está em voo, o `carregar()` do `onCriada` (fechado sobre o tipo ANTIGO) pode disparar e RESOLVER depois do `carregar()` do filtro novo, sobrescrevendo a tela com dado do filtro errado — ordem de EMISSÃO não é um proxy confiável pra "resposta mais recente" aqui. Corrigido com um `filtroAtualRef` que compara, no momento em que CADA resposta chega, se ela ainda corresponde ao filtro ATUALMENTE selecionado (não a um contador de "última requisição disparada") — descarta silenciosamente respostas pra filtros que o usuário já trocou. E2E dos 3 fluxos que criam mentoria e trocam o filtro na sequência (`mentorados.spec.ts` x2, `dashboard-admin.spec.ts` H10) reforçados com `waitForResponse` explícito no POST de criação + no GET disparado por ele + no GET do filtro, todos registrados ANTES da ação que os dispara (outro achado ao vivo: registrar depois faz a promise esperar pra sempre se a resposta já tiver chegado). 529/529 testes backend, `tsc -b` limpo, suíte ampla filtrada (113 testes, exclui specs mentee-facing/Loja pausados por `AREA_MENTORADO_PAUSADA`) rodada 2x numa base fresca: 85/113 e depois 84/113 — as ~27-28 falhas são 100% da classe já conhecida (mentee-facing pausado), **exceto uma flake isolada e não-relacionada** achada durante a verificação: `mentorados.spec.ts` "upload de áudio" falhou 2 de 3 vezes checando o conteúdo das sugestões da IA (`AtaEncaminhamentoSugerido`) — sintoma variou entre as tentativas (array vazio, 1 de 2 itens), indicando uma race de timing no pipeline Ata/IA (`AtaProcessamentoService`/polling de `AtaDetalhePage`) que este módulo não toca e não introduziu; registrado aqui pra investigação futura, não bloqueou a conclusão da Fase D.

**Fase F concluída** — "colar transcrição do Google Meet" adicionada como alternativa aditiva ao upload de áudio (Whisper), nunca substitutiva — o mentor escolhe qual usar via abas na mesma seção (renomeada de "Áudio da mentoria" pra "Transcrição da mentoria"). Backend: `AtaService.iniciarComTranscricaoColada` (mesma garantia de `afterCommit` de `iniciarUpload`, mesma corrida de `ObjectOptimisticLockingFailureException` evitada) chama `Ata.iniciarProcessamento(null)` (sem `audioUrl`) e agenda `AtaProcessamentoService.processarTranscricaoColada` — que pula `TranscricaoService`/`AudioStorageService` por completo e vai direto pra `AtaRascunhoService.gerarRascunho()` (mesmo `ClaudeAtaRascunhoService`, mesmo executor assíncrono). Novo endpoint `POST /admin/mentorias/{id}/ata/transcricao` (`ColarTranscricaoRequest`, `202 Accepted`, mesmo formato de resposta do upload). Frontend: `AtaDetalhePage.tsx`'s `UploadAudioForm` ganhou abas "Subir áudio"/"Colar transcrição" (`role="tablist"`), cada uma com seu próprio submit, convergindo no mesmo polling de status já existente. 531/531 testes backend, `tsc -b` limpo, E2E: novo teste completo (lead→mentorado→mentoria→realizar→colar transcrição→IA conclui→publica, mesmo pipeline do Claude via stub que o teste de áudio já cobria) rodado 2x consecutivas sem falha, mais o teste de áudio original intacto.

**Fase G concluída — última do M28, módulo inteiro fechado.** "Controle de vagas em evento por mentorado da Contínua (3 grátis/ano)": como investigado desde o início desta leva, não existia NENHUM jeito de inscrever um mentorado num evento além do self-service (`EventoMentoradoController`), hoje inalcançável (`AREA_MENTORADO_PAUSADA`) — esta fase criou o caminho admin do zero, com a cota aplicada nos dois caminhos (não é regra "só quando é admin"). Backend: `EventoMentoradoService` refatorado — `inscrever`/`cancelar` (self-service) e `inscreverAdmin`/`cancelarAdmin` (novo, mentoradoId por path param) convergem no mesmo núcleo (`inscreverNucleo`/`cancelarNucleo`), garantindo que a cota vale pra ambos. Cota: janela rolante de 12 meses a partir de `dataFechamentoContrato` (ano de CONTRATO, não civil — decisão confirmada com o Marcos), achando o ciclo atual (`ChronoUnit.YEARS.between` + `plusYears`) e contando inscrições ativas (`status != CANCELADA`) de eventos dentro desse ciclo; bloqueia a 4ª com `IllegalStateException` clara. Sem `dataFechamentoContrato` não bloqueia (dado incompleto não é culpa do mentorado). `EventoInscricaoAdminController` novo (`/admin/mentorados/{id}/eventos`, `Modulo.MENTORADOS` — não `CONTEUDOS`, é ação sobre o mentorado): `GET /inscricoes` (histórico), `GET /cota` (resumo pronto pro card "X/3", cálculo centralizado no backend — nunca duplicar a janela rolante no frontend), `POST`/`DELETE /{eventoId}/inscricao`. **Achado ao vivo, corrigido**: `LazyInitializationException` em `InscricaoEvento.evento` (LAZY) — `findByMentoradoId` só é seguro pra ler `.getId()` (não dispara load, vem do FK do proxy); qualquer leitura de título/data/tipo precisa de `buscarPorMentoradoComEvento` (novo, `LEFT JOIN FETCH`), mesma classe de bug já documentada em `MentoriaRepository`. Frontend: nova seção "Eventos" em `MentoradoDetalhePage.tsx` — resumo de cota (só quando aplicável), dropdown de inscrição (exclui eventos já ativos), histórico com pill de status + cancelar. 546/546 testes backend (14 novos: cota bloqueia/permite/ignora ciclo anterior/ignora canceladas/sem data não bloqueia/tipo diferente não bloqueia, caminho admin, consulta de cota), `tsc -b` limpo, E2E: spec novo `evento-inscricao-admin-m28.spec.ts` (2/2 — inscrever/cancelar/reabrir vaga, e cota bloqueando o 4º evento com 4 eventos + mentorado próprios criados no teste, sem depender de fixture do seed), confirmado end-to-end contra o dev real via screenshot (inscrever → Inscrito → cancelar → Cancelada → vaga reaberta no dropdown). **Suíte ampla final (116 testes) rodada 1x limpa numa base fresca: 89/116, as 27 falhas são 100% da classe conhecida (mentee-facing pausado) — zero regressão, e a flake isolada de sugestões de IA achada durante a Fase D não se repetiu.**

### M28 — resumo final (todas as 7 fases concluídas, 21/07/2026)
Import único de mentorados (A) · lista de seleção vertical (B) · página dedicada de mentorado estilo Notion (C) · mentorias reorganizadas por tipo (D) · rename Gestão de Performance (E) · colar transcrição do Meet (F) · cota de eventos + inscrição admin (G). Nada commitado ainda — aguardando revisão do Marcos no dev antes do commit/push (mesmo fluxo de sempre desta esteira: implementar → testar → apresentar → só commitar com confirmação explícita).
