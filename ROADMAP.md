...
| 20 | E14/E17 · Financeiro & Painel Consolidado — itens do "Sequenciamento sugerido" da reunião 17/07/2026 (change request pós-MVP) | Médio (parcial) | — | 🟡 **Parcial (19/07/2026)** — 4 de 7 itens do E14 fechados: filtro mensal em contas a pagar/receber (`ContaPagarReceberRepository.buscarComFiltro`, sentinela `[1900-01-01, 2999-12-31)` em vez de parâmetro nulo — Postgres não infere tipo de `LocalDate` nulo numa query com tantas colunas `bytea` de pgcrypto ao redor), rename da aba "Faturamento"→"Dashboard", conciliação contrato×recebido (`ConciliacaoService`/`ConciliacaoController`, novo, vive em `comercial` mas gated `Modulo.FINANCEIRO` — mesmo padrão de `MentoradoContratoController`/M23), e "alimentação automática de contas via venda" (já vinha do M25). 1 de 4 itens do E17 fechado: campo `Ata.decisoes` (manual + extraído por IA via `ClaudeAtaRascunhoService`, schema de tool-use estendido). 494/494 testes, `tsc -b` limpo, `revisor-seguranca` rodado. **Ainda pendente** (documentado em detalhe em `docs/reuniao-2026-07-17-atualizacoes.md` §§ E14/E17): subcategorias/fixo-variável em `CategoriaFinanceira`, merge lançamentos/contas, evento rastreado no financeiro, controle de presença em mentoria, ranking com as 4 ferramentas nomeadas (substitui `ferramentasConcluidas`/`ferramentasTotal` já em produção), dois eixos de acompanhamento (engajamento + risco de churn, substitui o status único `EM_DIA`/`ATENCAO`/`ATRASADO`) — os 3 últimos mexem em lógica de ranking/status já em produção, maior risco, merecem confirmação de produto antes de implementar |
