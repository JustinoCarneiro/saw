...
class MentoradoConsolidadoResponseTest {
    ...
    void ferramentasPctSegueAMesmaLogicaDeArredondamento() {
        // 2/3 = 66.66...% -> 67
        var response = MentoradoConsolidadoResponse.from(row(1, 1, 2, 3));
        assertThat(response.ferramentasPct()).isEqualTo(67);
    }

    @Test
    void status_emDia_a_partir_de_60_por_cento_inclusive() {
        assertThat(MentoradoConsolidadoResponse.from(row(60, 100, 0, 1)).status()).isEqualTo("EM_DIA");
        assertThat(MentoradoConsolidadoResponse.from(row(59, 100, 0, 1)).status()).isEqualTo("ATENCAO");
    }

    @Test
    void status_atencao_a_partir_de_30_por_cento_inclusive() {
        assertThat(MentoradoConsolidadoResponse.from(row(30, 100, 0, 1)).status()).isEqualTo("ATENCAO");
        assertThat(MentoradoConsolidadoResponse.from(row(29, 100, 0, 1)).status()).isEqualTo("ATRASADO");
    }

    @Test
    void status_atrasado_abaixo_de_30_por_cento() {
        assertThat(MentoradoConsolidadoResponse.from(row(0, 100, 0, 1)).status()).isEqualTo("ATRASADO");
    }

    @Test
    void status_100_por_cento_e_em_dia() {
        assertThat(MentoradoConsolidadoResponse.from(row(10, 10, 3, 3)).status()).isEqualTo("EM_DIA");
    }
    ...
    void mantemOsCamposCrusDaRowQueNaoSaoRecalculados() {
        ...
        var linha = new MentoradoConsolidadoRow(id, "João Silva", "Restaurante Sabor & Arte",
                new BigDecimal("18.0"), 3, 3, 10L, 9L, 10L, 9L);

        var response = MentoradoConsolidadoResponse.from(linha);
