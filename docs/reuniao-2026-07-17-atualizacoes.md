...
- 🔵 **Implementado (19/07/2026, M27) — Lista de "ferramentas obrigatórias" nomeadas**: DRE
  estruturada, manual de cultura, ficha técnica, manual de processos — cada uma com 3 estados
  (sim/não/em construção, mesmo padrão de `culturaConstruida`/`processosDesenhados` do Diagnóstico
  Inicial). As 4 pesam igual; "em construção" conta como não-concluída (decisão tomada sem
  resposta do Victor, ver "Perguntas" abaixo — pode mudar). `ferramentasConcluidas`/
  `ferramentasTotal` **continuam existindo** (não removidos), mas passam a ser **derivados** das 4
  em vez de dois inteiros editáveis livremente — nada que já lê esses dois campos quebra.
- 🔵 **Implementado (19/07/2026, M27) — controle de presença em mentoria/aula**: nova entidade
  `PresencaMentoria`, só pra mentoria **coletiva** (individual já é coberta pelo status da sessão).
  O mentor/time com acesso a Mentorados marca presença por mentorado, depois da sessão — vira uma
  métrica nova e separada ("frequência: X/Y mentorias"), **não muda o cálculo do ranking/
  percentual de execução** hoje em produção (decisão tomada sem resposta do Victor — pode mudar).
- 🔵 **Implementado (19/07/2026, M27) — dois eixos de acompanhamento** (Nível de engajamento:
  Alto/Médio/Baixo + Risco de churn: Não/Atenção/Alto, achado na página "PADRONIZAÇÕES"),
  preenchidos manualmente pelo mentor/time de sucesso — sem fórmula automática, não havia dado
  confiável pra uma. O status único atual (`EM_DIA`/`ATENCAO`/`ATRASADO`, calculado a partir do
  progresso em `ConsolidatedService`) **continua existindo do mesmo jeito** — os dois eixos
  aparecem ao lado, só no Painel Consolidado (não no Dashboard individual do mentorado nesta leva).
...
**Atualização (19/07/2026, mesma noite):** mensagem enviada ao Victor com as perguntas abaixo,
ainda sem resposta. O Marcos autorizou implementar os 3 itens agora com a decisão de produto que
fizer mais sentido, com o entendimento explícito de que pode precisar ajuste quando o Victor
responder — por isso o M27 (Blueprint completo mais abaixo) foi desenhado **deliberadamente
aditivo**: nada do que já está em produção (ranking, status EM_DIA/ATENCAO/ATRASADO,
`ferramentasConcluidas`/`ferramentasTotal`) muda de comportamento por padrão, só ganha informação
nova ao lado. As decisões tomadas estão registradas em cada bullet da seção anterior — se a
resposta do Victor divergir de alguma, o ajuste é local (não deve exigir redesenho).
...
3. ✅ **Financeiro (E14)** — 7 de 7 itens resolvidos (19/07/2026): filtro mensal, rename da aba,
   conciliação, alimentação automática de contas via parcela (já vinha do M25), evento rastreado
   no financeiro, subcategorias/fixo-variável em `CategoriaFinanceira` e o merge
   `ContaPagarReceber`→`LancamentoFinanceiro` (M26).
4. 🔵 **Mentorado/E17** — 4 de 4 itens implementados (19/07/2026): campo Decisões na ata (manual e
   via IA) + os 3 itens de maior risco do M27 (controle de presença em mentoria coletiva, ranking
   com as 4 ferramentas nomeadas, dois eixos de acompanhamento) — implementados **sem** resposta
   do Victor às perguntas enviadas (autorização explícita do Marcos: "responda com o que fizer
   mais sentido, ajustamos depois se não for o que ele quer"), por isso desenhados 100% aditivos
   (nada do que já está em produção muda de comportamento por padrão). Ver seção "E17" acima pras
   decisões tomadas em cada um — sujeitas a ajuste quando o Victor responder.
